...
def main(vcf_path, gtf_path, fasta_path):
    ...
    if is_single_file:
        ...
        print(f"Processing directory with {len(vcf_files)} VCF files")

    print(vcf_files[0])

    vcf = read_vcf(vcf_files[0])
    print(f"VCF File shape: {vcf.shape}")
    ...
    # analyze GTF --> could put this into an function?
    ...
    # Apply the NMD rules
    # TODO: update the rules so they consider where the variant is lying
